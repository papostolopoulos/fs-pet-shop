...
app.put('/pets/:id/:age/:kind/:name', function (req, res) {
  fs.readFile(petsPath, "utf8", function (readErr, petsJSON) {
    if (readErr) {
      console.error(readErr.stack);
      return res.sendStatus(500);
    ...
    pet.age = age;
    pet.kind = kind;
    if (!pet.name || !pet.age || !pet.kind || id < 0 || id > pets.length || isNaN(id)) {
      console.log("in the if for the PUT");
      return res.sendStatus(400);
    }

    pets[id] = pet;

    var newPetsList = JSON.stringify(pets);

    fs.writeFile(petsPath, newPetsList, function(writeErr) {
      if (writeErr) {
        console.error(writeErr.stack);
        return res.sendStatus(500);
      }
...
app.delete('/pets/:id', function (req, res) {
  fs.readFile(petsPath, "utf8", function (readErr, petsJSON) {
    ...
    if (id < 0 || id > pets.length || isNaN(id)) {
      console.log("in the if for the delete");
      return res.sendStatus(400);
    }

    var deletedPet = pets.splice(id, 1)
...
app.listen(port, function() {
  console.log("Listening on port", port, "and for project fs-pet-shop (Rest Express Server)");
});
